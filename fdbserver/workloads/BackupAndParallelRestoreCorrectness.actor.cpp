	...
	ACTOR static Future<Void> _start(Database cx, BackupAndParallelRestoreCorrectnessWorkload* self) {
		...
		try {
			...
			if (lastBackupContainer && self->performRestore) {
				...
				// choosing a random target version here
				// state Version targetVersion = pickRandomTargetVersion(desc);
				...
				// use backup ranges as restore ranges
				// restore to what database? the original database?
				// where do we do the consistency check?
				...
				std::vector<FileBackupAgent::ParallelRestoreRequest> restoreRequests;
				// Make it very likely to split the key ranges.
				if (deterministicRandom()->random01() < 0.1) {
