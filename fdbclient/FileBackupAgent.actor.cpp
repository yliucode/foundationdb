...
public:
	...
	ACTOR static Future<Void> submitParallelRestore(Database cx, Key backupTag,
	                                                std::vector<FileBackupAgent::RestoreKeyRangesVersion> requests,
	                                                Key bcUrl, bool lockDB, UID randomUID, Key addPrefix,
	                                                Key removePrefix) {
			...
			try {
				...
				for (int i = 0; i < requests.size(); i++) {
					for (const auto& range : requests[i].first) {
						...
						struct RestoreRequest restoreRequest(restoreIndex, restoreTag, bcUrl, actualTargetVersions[i],
						                                     range, deterministicRandom()->randomUniqueID(), addPrefix,
						                                     removePrefix);
	...
	ACTOR static Future<Version> atomicRestore(FileBackupAgent* backupAgent, Database cx, Key tagName,
		...
		if (fastRestore) {
			...
			bool lockDB = true;
			wait(submitParallelRestore(cx, tagName, { { ranges, targetVersion } }, KeyRef(bc->getURL()), lockDB,
...
Future<Void> FileBackupAgent::submitParallelRestore(Database cx, Key backupTag,
	...
	return FileBackupAgentImpl::submitParallelRestore(cx, backupTag, requests, bcUrl, lockDB, randomUID, addPrefix,
	                                                  removePrefix);
}
